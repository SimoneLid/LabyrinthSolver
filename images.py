...
def load_RGB(filename):
    """ Carica l'immagine PNG dal file 'filename'.  Torna una lista di liste di pixel.
...
def load_RGBA(filename):
    """ Carica l'immagine PNG dal file 'filename'.  Torna una lista di liste di pixel.
        Ogni pixel è una tupla (R, G, B, A) dei 3 colori e del valore alpha con valori tra 0 e 255.
        Load a PNG image from file 'filename'. Return a list of lists of pixel.
        Each pixel is a tuple (R, G, B, A) of its 3 colors and alpha channel with values in 0..255.
    """
    with open(filename, mode='rb') as f:
        reader = png.Reader(file=f)
        w, h, png_img, _ = reader.asRGBA8()
        # ottimizzata leggermente
        w *= 4
...
def load_RGBA_notA(filename):
    """ Carica l'immagine PNG dal file 'filename'.  Torna una lista di liste di pixel eliminando il valore alpha.
        Ogni pixel è una tupla (R, G, B) dei 3 colori.
        Load a PNG image from file 'filename'. Return a list of lists of pixel without alpha channel.
        Each pixel is a tuple (R, G, B) of its 3 colors.
    """
    with open(filename, mode='rb') as f:
        reader = png.Reader(file=f)
        w, h, png_img, _ = reader.asRGBA8()
